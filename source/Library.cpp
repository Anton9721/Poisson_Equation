...
std::vector<double> data_potential(std::string solver_name, std::span<double> boundary_conditions, 
                                ...
                                const int iteration, const double precision, const double omega)
{
    ...
    if (solver_name == "Solver_Jacobi")
    {
        ...
        solver.solve(potential_data, boundary_conditions, source2D, nod_number, iteration, precision, omega);
        for (size_t i = 0; i < nod_number; ++i){
            for (size_t j = 0; j < nod_number; ++j){
    ...
    else if (solver_name == "Solver_Gauss")
    {
        Solver_Gauss solver;
        solver.solve(potential_data, boundary_conditions, source2D, nod_number, iteration, precision, omega);
        for (size_t i = 0; i < nod_number; ++i){
    ...
    else if (solver_name == "Solver_over_relaxation")
    {
        Solver_over_relaxation solver;
        solver.solve(potential_data, boundary_conditions, source2D, nod_number, iteration, precision, omega);
        for (size_t i = 0; i < nod_number; ++i){
            for (size_t j = 0; j < nod_number; ++j){
                data_result.push_back(potential_data(i, j));
